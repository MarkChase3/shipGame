...
function shootUpdate(shoot, i, f) {
  ...
  ctx.fill();
  if(shoot.img!=false){
    ctx.drawImage(shoot.img,shoot.frame[0],shoot.frame[1],shoot.frame[2],shoot.frame[3],shoot.x - shoot.frame[2]/2, shoot.y - shoot.frame[3]/2,shoot.frame[2],shoot.frame[3]);
  }
  f(shoot,i);
}
