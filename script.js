...
//enemies
let enemies = [];
function createEnemie(imgPath, y, speed) {
  let enemie = {};
...
function enemieDontCollideWithOthers(enemie1, i) {
...
function enemiesFollowPlayer() {
 ...
 for(let i=0;i<enemies.length;i++){
  if (enemies[i].x <= canvas.width / 10 * 9 && Math.abs(player.y - enemies[i].y)>enemies[i].speed) {
      ...
      if (!enemieDontCollideWithOthers(enemies[i], i, i)) {
        enemies[i].y = yBuffer;
      }
  }
}
...
function createPlayer() {
  ...
  player.frames = [[1, 1, 16, 16], [18, 1, 16, 16]];
  player.currFrame = 0;
  player.lastFrame = 0;
  player.frameChangeFrequency = 100;
...
update();
enemies.push(createEnemie('enemie1Ship.png', 80, 2))
enemies.push(createEnemie('playerShip.png', 50, 4))